...
*/
public class PlayerGS
{
  
  // attributes for the player's inventory
  private ArrayList<LakeObjectGS> inventory;
  ...
  public PlayerGS()
  {
    ...
    inventory = new ArrayList<LakeObjectGS>(); // 16 spaces for inventory
    inventory.add(new WalletGS()); // Wallet must be the first item
    inventory.getFirst().setCost(cash);
    inventory.add(new HookGS());
    inventory.add(new BaitGS());
    maxInventory = 16;
  ...
  public ArrayList<LakeObjectGS> getInventory()
  {
    return inventory;
  }
  ...
  { 
    ...
    for (LakeObjectGS item : inventory)
    {
      if (item == null)
        hasHook = false;
   ...
   */
  public HookGS getStrongestHook()
  { 
    HookGS strongestHook = null;
   ...
   */
  public LakeObjectGS getItem(int index)
  {
   ...
   */
  public void updateInventory(LakeObjectGS newItem, boolean toAdd)
  {
